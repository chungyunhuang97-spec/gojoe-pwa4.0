...
export const Training: React.FC = () => {
  ...
  useEffect(() => {
    const existing = workoutLogs.find((l: WorkoutLog) => l.date === dateKey);
    if (existing) {
  ...
  const handleDeleteExercise = (exId: string) => {
      if (!activeLog) return;
      const updated = activeLog.exercises.filter((e: WorkoutExercise) => e.id !== exId);
  ...
  const toggleBodyPart = (part: string) => {
      ...
      const updated = current.includes(part) ? current.filter((p: string) => p !== part) : [...current, part];
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6">
            ...
            <div className="space-y-3">
                ...
                {activeLog && activeLog.exercises.length > 0 ? (
                    activeLog.exercises.map((ex: WorkoutExercise, idx: number) => (
